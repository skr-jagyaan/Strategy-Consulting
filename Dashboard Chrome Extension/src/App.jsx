...
export default function App() {
  ...
  const simulationData = {
    week3: {
      ...
      winRate: 18,
      winRateHistory: [16, 19, 17, 18], 
      focus: 92,
      contradictions: 1,
      mandate: [
        "Logic Holding: Prospective data supports the premium assumption.",
        "Maintain focus: Team is successfully avoiding 'Special Cause' noise.",
        "Stay the course: No strategic intervention required."
      ],
    ...
    week6: {
      ...
      mandate: [
        ...
        "Pivot Required: Re-evaluate the 'Barrier to Choice' immediately."
      ],
  ...
  const handleLoadCsv = async () => {
    ...
    try {
      ...
      if (allLines.length < 4) throw new Error("CSV does not match Elite Layout (Company Name in B1, Data from Row 4).");

      const row1Values = allLines[0].split(/,(?=(?:(?:[^"]*"){2})*[^"]*$)/);
      let compName = (row1Values.length > 1 && row1Values[1]) ? row1Values[1].replace(/^"|"$/g, '').trim() : "Unnamed Strategy";
      ...
      if (dataLines.length === 0) throw new Error("No data rows found below headers.");
  ...
  // --- ENTERPRISE CHART SCALING ---
  // Win Rate Trend (Maps values to a 0-100 SVG coordinate system securely)
  const maxScale = Math.max(...current.winRateHistory, 30); // Ensures chart doesn't clip if rate is low
  ...
  const isRedState = current.winRate < 10 || current.variance === 'Special Cause';
  const isYellowState = !isRedState && current.winRate < 15;
  const trendColor = isRedState ? "#f43f5e" : (isYellowState ? "#f59e0b" : "#10b981");

  // Circular Donut Chart Logic
  const radius = 40;
  const circumference = 2 * Math.PI * radius;
  const strokeDashoffset = circumference - (current.focus / 100) * circumference;
  const focusColor = current.focus < 60 ? "text-rose-500" : (current.focus < 75 ? "text-amber-500" : "text-emerald-500");
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white p-2 rounded-2xl flex flex-col md:flex-row gap-2 items-center shadow-sm border border-slate-200">
          ...
          <button 
            ...
            className="w-full md:w-auto bg-slate-900 hover:bg-slate-800 text-white px-8 py-2.5 rounded-xl text-sm font-semibold flex items-center justify-center transition-colors"
          >
        ...
        {error && (
          <div className="p-4 bg-rose-50 text-rose-700 rounded-xl flex items-center border border-rose-200">
            <AlertCircle className="mr-3 w-5 h-5 shrink-0"/>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 px-2">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium mt-1">
              90-Day Strategy Monitor &nbsp;•&nbsp; Commenced: {current.startDate}
            </p>
          </div>
          <div className="relative">
            <select 
              className="appearance-none bg-white border border-slate-200 py-2 pl-4 pr-10 rounded-lg text-sm font-semibold text-slate-700 shadow-sm cursor-pointer hover:border-slate-300 transition-colors"
              value={liveData ? 'live' : week} 
              onChange={e => e.target.value !== 'live' && (setLiveData(null), setWeek(e.target.value))}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center">
              <Search className="w-3.5 h-3.5 mr-1.5 text-slate-400"/> Core WWHTBT
            </span>
            <p className="text-sm font-semibold text-slate-700 leading-snug flex-1">{current.wwhtbt}</p>
          </div>
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center">
              <FileText className="w-3.5 h-3.5 mr-1.5 text-slate-400"/> Skeptic's Contract
            </span>
            <p className="text-sm font-semibold text-slate-700 flex-1">{current.limits}</p>
          </div>
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center">
              <Zap className="w-3.5 h-3.5 mr-1.5 text-slate-400"/> SPC Variance Check
            </span>
            <div className="flex items-center">
              <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide
                ${current.variance === 'Special Cause' ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-700'}`}>
                {current.variance === 'Special Cause' ? <TrendingDown className="w-3 h-3 mr-1"/> : <TrendingUp className="w-3 h-3 mr-1"/>}
                {current.variance}
              </span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
          ...
          {/* LEFT COLUMN: WIN RATE TRENDLINE (Spans Full Height) */}
          <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm flex flex-col h-full">
            <div className="flex justify-between items-start mb-2">
              <div>
                ...
                <p className="text-sm text-slate-500 font-medium">Rolling 4-Week Performance Tracking</p>
              </div>
              <div className="text-right">
                <span className={`text-4xl font-bold tracking-tight ${isRedState ? 'text-rose-600' : (isYellowState ? 'text-amber-500' : 'text-slate-900')}`}>
                  {current.winRate}%
                </span>
              </div>
            </div>
          ...
          {/* RIGHT COLUMN: STACKED FOCUS AND ANOMALIES */}
          <div className="flex flex-col gap-6 h-full">
            
            {/* TOP CARD: STRATEGIC FOCUS */}
            <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm flex-1 flex flex-col items-center justify-center min-h-[180px]">
              <div className="w-full flex justify-between items-start mb-2">
                 <h3 className="text-sm font-bold text-slate-900">Strategic Focus</h3>
              ...
              <div className="relative w-28 h-28 flex-shrink-0 mt-2">
                ...
                <svg viewBox="0 0 100 100" className="w-full h-full transform -rotate-90">
                  ...
                  <circle 
                    ...
                    strokeLinecap="round"
                    className={`${focusColor} transition-all duration-1000 ease-out`} 
                  />
                </svg>
                {/* Center Text */}
            ...
            <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm flex-1 flex flex-col justify-between min-h-[180px]">
              <div className="flex justify-between items-start mb-4">
                 <h3 className="text-sm font-bold text-slate-900">Market Anomalies</h3>
                 <span className="text-[10px] font-semibold text-slate-500 bg-slate-100 px-2 py-1 rounded">Limit: 5</span>
              </div>
              
              {/* The LED Track */}
              <div className="flex gap-1.5 h-8 mb-4 w-full">
                {[1, 2, 3, 4, 5].map((level) => {
                  let bgColor = "bg-slate-100"; // Empty state
                  ...
                  return (
                    <div 
                      key={level} 
                      className={`flex-1 rounded-sm border border-black/5 transition-all duration-500 ${bgColor}`} 
                    />
                  );
                })}
              </div>
              ...
              <div className="flex items-center gap-2 mt-auto">
                <span className={`text-4xl font-bold tracking-tight leading-none ${current.contradictions >= 5 ? 'text-rose-600' : 'text-slate-900'}`}>
        ...
        {/* EXECUTIVE MANDATE PANEL */}
        <div className={`p-6 rounded-3xl border-l-4 shadow-sm transition-colors ${current.mandateColors}`}>
          <h3 className="text-xs font-bold uppercase tracking-widest mb-4 flex items-center opacity-80">
            <Activity className="w-4 h-4 mr-2"/> Recommended Action
          </h3>
          <ul className="space-y-3">
            {current.mandate.map((m, i) => (
              <li key={i} className="flex items-start text-sm font-medium">
                <span className="w-1.5 h-1.5 rounded-full bg-current opacity-50 mt-1.5 mr-3 shrink-0" />
